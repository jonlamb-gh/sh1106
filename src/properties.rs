    ...
    DI: DisplayInterface,
{
    ...
    pub async fn init_column_mode(&mut self) -> Result<(), DI::Error> {
        ...
        match self.display_size {
            DisplaySize::Display128x32 => Command::ComPinConfig(false).send(&mut self.iface).await,
            DisplaySize::Display128x64
            | DisplaySize::Display128x64NoOffset
            | DisplaySize::Display132x64 => Command::ComPinConfig(true).send(&mut self.iface).await,
        }?;
